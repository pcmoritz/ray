...
static PyObject* is_object_ready(PyObject* self, PyObject* args) {
  Worker* worker;
  ObjectID objectid;
  if (!PyArg_ParseTuple(args, "O&O&", &PyObjectToWorker, &worker, &PyObjectToObjectID, &objectid)) {
    return NULL;
  }
  bool is_ready = worker->is_object_ready(objectid);
  if (is_ready){
    Py_RETURN_TRUE;
  }
  else {
...
static PyMethodDef RayLibMethods[] = {
 ...
 { "is_object_ready" , is_object_ready, METH_VARARGS, "checks the scheduler to see if a object can be gotten" },
